...
CKPT="${AUTOVLA_ROOT}/runs/sft/2026-05-24_19-17-19/epoch=2-loss=0.9322.ckpt"
OUT_DIR="${REPO_ROOT}/outputs/autovla/embeddings"
LOG_DIR="${REPO_ROOT}/runs"
MODE="autoregressive" # "teacher-forcing"  # or "autoregressive"
...
extract() {
    ...
    nohup python "${REPO_ROOT}/src/extraction/autovla/extract.py" \
        ...
        --device "${device}" \
        > "${LOG_DIR}/${timestamp}_extract_${MODE}_${split}.log" 2>&1 &
}
